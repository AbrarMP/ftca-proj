                  ...
                  bitmap_info_header_t *bitmapInfoHeader)
{
    ...
    pad = 4*ceil(bitmapInfoHeader->bitspp*bitmapInfoHeader->width/32.) - 4*bitmapInfoHeader->bitspp*bitmapInfoHeader->width/32.;
    for(size_t i=0; i<bitmapInfoHeader->height; i++){
	    for(size_t j=0; j<bitmapInfoHeader->width; j++){
		    ...
		    red = (pixel_t) c;
		    bitmapImage[count++] = (pixel_t) (0.2126*red + 0.7152*green + 0.0722*blue);
	    }
              ...
              const pixel_t *data)
{
    ...
    // Palette/Color Table
    // Omitted since it can make a bitmap quite large bitspp=24 --> 64MB
/*
...
*/
    ...
    int channels;
    size_t pad = 4*ceil(bmp_ih->bitspp*bmp_ih->width/32.) - 4*bmp_ih->bitspp*bmp_ih->width/32.;
    unsigned char c;
...
// in 			=> input array of pixels
// out 			=> output array of pixels
// kernel 		=> kernel array
// nx			=> pixel width
// ny			=> pixel height
// kn			=> for a kernel kn x kn
// normalize	=> normalization enabled for gaussian filter
